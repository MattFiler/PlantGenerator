...
public:
	...
	/* Save a LoadedModel out to an OBJ and MTL */
	bool SaveModel(LoadedModel model_data, std::string path) 
	{
		std::ofstream myfile;
		myfile.open(path);
		if (!myfile.is_open()) return false;
		myfile << "#### AUTOGENERATED BY PLANTGENERATOR ####\n";
		myfile << "mtllib GeneratorOutput.mtl\n";
		int offset = 0;
		for (int i = 0; i < model_data.modelParts.size(); i++) {
			myfile << "o part_" << std::to_string(i) << "\n";
			for (int x = 0; x < model_data.modelParts[i].compVertices.size(); x++) {
				XMFLOAT3 thisVert = model_data.modelParts[i].compVertices[x].Pos;
				myfile << "v " << std::to_string(thisVert.x) << " " << std::to_string(thisVert.y) << " " << std::to_string(thisVert.z) << "\n";
			}
			for (int x = 0; x < model_data.modelParts[i].compVertices.size(); x++) {
				XMFLOAT2 thisCoord = model_data.modelParts[i].compVertices[x].Tex;
				myfile << "vt " << std::to_string(thisCoord.x) << " " << std::to_string(thisCoord.y) << "\n";
			}
			for (int x = 0; x < model_data.modelParts[i].compVertices.size(); x++) {
				XMFLOAT3 thisNorm = model_data.modelParts[i].compVertices[x].Normal;
				myfile << "vn " << std::to_string(thisNorm.x) << " " << std::to_string(thisNorm.y) << " " << std::to_string(thisNorm.z) << "\n";
			}
			myfile << "usemtl " << model_data.modelParts[i].thisMaterial.materialName << "\n";
			for (int x = 0; x < model_data.modelParts[i].compIndices.size(); x+=3) {
				myfile << "f " << model_data.modelParts[i].compIndices[x] + 1 + offset << "/" << model_data.modelParts[i].compIndices[x] + 1 + offset << "/" << model_data.modelParts[i].compIndices[x] + 1 + offset << " ";
				myfile << model_data.modelParts[i].compIndices[x+1] + 1 + offset << "/" << model_data.modelParts[i].compIndices[x+1] + 1 + offset << "/" << model_data.modelParts[i].compIndices[x+1] + 1 + offset << " ";
				myfile << model_data.modelParts[i].compIndices[x+2] + 1 + offset << "/" << model_data.modelParts[i].compIndices[x+2] + 1 + offset << "/" << model_data.modelParts[i].compIndices[x+2] + 1 + offset << "\n";
			}
